...
export default {
  config: {
    ...
    sofiDefine: {
      title: "SOFiSTiK: Show sections inside define block",
      type: 'boolean',
      default: false,
      description: 'You can outline section inside define block or not. This tick work only in SOFISTiK and SOFISTiK-RAW grammars. In another doesn\'t have any effects.',
      order: 6,
    }
  },
  ...
  activate(state) {
    ...
    // this.subscriptions.add(atom.config.observe('navigation-pane.sofiDefine', () => {
    //   this.updateCurrentEditor(atom.workspace.getActiveTextEditor());
    // }));
  ...
  sofiDefineToggle() {
    if (atom.config.get('navigation-pane.sofiDefine')===true) {
      atom.config.set('navigation-pane.sofiDefine', false)
    } else {
      atom.config.set('navigation-pane.sofiDefine', true)
    }
    this.update(atom.workspace.getActiveTextEditor())
  },
