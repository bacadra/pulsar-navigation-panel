...
class NavigationTreeView {
  ...
  render() {
    ...
    if (this.childOutlines && this.showChildren) {
      sublist = <ol class="list-tree">
          {this.childOutlines.map(child => {
            return <NavigationTreeView {...child}/>;
          })}
        </ol>;
    }
    ...
    let itemId = `navigation-pane-${this.startRow}-${this.endRow}`;
  ...
  didClick() {
    ...
    if (editor && documentPos) {
      ...
      editor.scrollToCursorPosition({center: atom.config.get('navigation-pane.general.centerScroll')});
    }
...
module.exports = {NavigationTreeView};
