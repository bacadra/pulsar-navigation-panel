...
class AbstractModel {
  ...
  _stackHeadings(rawHeadings) {
    ...
    for (let heading of rawHeadings) {
      ...
      if (this.markLines) {
        this.editor.getNavigationMarkerLayer(heading.revel).markBufferRange(
            heading.headingRange, {exclusive:true, invalidate:'inside'}
        )
      }
    }

    if (top) { // fix last elements range
      top = stack[0]
      while (top.children.length) {
        top = top.children[top.children.length-1]
        if (top.children.length) {
          top.endPosition = Point.INFINITY
          top.range.end = Point.INFINITY
        } else {
          break
        }
      }
  ...
  _parseLevelFast(start, end, level) {
    ...
    for (let headingRange of ranges) {
      ...
      if (result) {
        ...
        if (parsedResult) {
          if (parsedResult.level===undefined) {continue}
          let heading = {
