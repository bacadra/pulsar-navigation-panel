...
export default {
    ...
    this.subscription = this.subscription.then(() => {
      ...
      if (this.editor) {
        ...
        if (scopeName in SCANNERS) {
          ...
          this.scrollDispose = editor.onDidChangeScrollTop((e) => {
            if (this.headers) {
              this.updateByScroll()
            }
          })
  ...
  updateByScroll() {
    if (!this.editor.cursors.length) return
    const cursor = this.editor.cursors[0]
    this.clearCursorItems(cursor)
    this.findCursorItems(cursor, Math.round(this.editor.getScrollTop() / this.editor.lineHeightInPixels + this.editor.rowsPerPage / 4))
    this.navigationTree.update( this.headers )
  },
