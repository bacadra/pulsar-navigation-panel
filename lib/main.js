...
export default {
  ...
  editorUnsubscribe() {
    if (this.scanner) {
      ...
      this.scrollDispose .dispose() ; this.scrollDispose  = null
      ...
      if (this.editor) {
        ...
        if (scopeName in SCANNERS) {
          ...
          this.scrollDispose = atom.views.getView(editor).onDidChangeScrollTop((scrollTop) => {
            if (this.headers) { this.updateByScroll(scrollTop) }
          })
        }
      }
    }).then(() => {
  ...
  updateByScroll(scrollTop) {
    if (this.traceVisible) {
      this.setVisibleItem(this.headers,
        Math.round(scrollTop/this.editor.lineHeightInPixels),
        Math.round(scrollTop/this.editor.lineHeightInPixels+this.editor.rowsPerPage),
      ),
      this.navigationTree.update( this.headers )
    }
  },
