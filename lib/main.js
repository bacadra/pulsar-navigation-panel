...
export default {
  ...
  updateByScroll(scrollTop) {
    this.setVisibleItem(
      Math.round(scrollTop/this.editor.lineHeightInPixels + this.editor.rowsPerPage*0),
      Math.round(scrollTop/this.editor.lineHeightInPixels + this.editor.rowsPerPage*1),
    ),
    this.navigationTree.update( this.headers )
  },
  ...
  setVisibleItem(minRow, maxRow) {
    ...
    for (var i=0; i<headers.length-1; i++) {
      let row = headers[i].startPoint.row
