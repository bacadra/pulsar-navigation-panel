...
import { ScannerSinumerik } from "./scanners/sinumerik"
...
export default {
    ...
    this.subscription = this.subscription.then(() => {
      ...
      if (this.editor) {
        ...
        if (scopeName in SCANNERS) {
          ...
          this.scrollDispose = atom.views.getView(editor).onDidChangeScrollTop((scrollTop) => {
            this.updateByScroll(scrollTop)
          })
        }
      }
    }).then(() => {
      this.subscription = this.subscription
        .then(() => { this.update() })
        .then(() => { this.updateByScroll(atom.views.getView(this.editor).getScrollTop()) })
    })
  },

  updateByScroll(scrollTop) {
    if (this.traceVisible && this.headers) {
      this.setVisibleItem(this.headers,
        Math.round(scrollTop/this.editor.getLineHeightInPixels()),
        Math.round(scrollTop/this.editor.getLineHeightInPixels()+this.editor.rowsPerPage),
      ),
      this.navigationTree.update( this.headers )
    }
  },
  ...
  setVisibleItem(headers, minRow, maxRow) {
    for (var i=0; i<headers.length; i++) {
      let startRow = this.editor.screenPositionForBufferPosition(headers[i].startPoint).row
      let lastRow = this.editor.screenPositionForBufferPosition([headers[i].lastRow, 0]).row
      if ((minRow<=startRow && startRow<=maxRow) || (startRow<=minRow && minRow<=lastRow)) {
        headers[i].visibility = 1
      } else {
        headers[i].visibility = 0
      }
      this.setVisibleItem(headers[i].children, minRow, maxRow)
    }
  },
