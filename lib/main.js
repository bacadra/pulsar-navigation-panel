...
export default {
    ...
    this.subscription = this.subscription.then(() => {
      ...
      if (this.editor) {
        ...
        if (scopeName in SCANNERS) {
          ...
          this.scrollDispose = this.editorView.onDidChangeScrollTop((scrollTop) => {
  ...
  updateByScroll(scrollTop) {
    if (this.traceVisible && this.headers) {
      this.setVisibleItem(this.headers, scrollTop, scrollTop+this.editorView.getHeight()),
      this.navigationTree.update( this.headers )
    }
  },

  setVisibleItem(headers, minPx, maxPx) {
    for (var i=0; i<headers.length; i++) {
      let startPx = this.editorView.pixelPositionForBufferPosition(headers[i].startPoint).top
      let endPx = this.editorView.pixelPositionForBufferPosition([headers[i].lastRow, 0]).top
      if ((minPx<=startPx && startPx<=maxPx) || (startPx<=minPx && minPx<=endPx)) {
        headers[i].visibility = 1
      } else {
        headers[i].visibility = 0
      }
      this.setVisibleItem(headers[i].children, minPx, maxPx)
    }
  },
