...
export default {
  ...
  updateByScroll(scrollTop) {
    this.setVisibleItem(Math.round(scrollTop/this.editor.lineHeightInPixels + this.editor.rowsPerPage/4*0))
    this.navigationTree.update( this.headers )
  },
  ...
  setVisibleItem(visibleRow) {
    if (this.headers===null) { return }
    let headers = this.getFlattenHeaders()
    for (var i=0; i<headers.length-1; i++) {
      if (headers[i].startPoint.row<=visibleRow && visibleRow<headers[i+1].startPoint.row) {
