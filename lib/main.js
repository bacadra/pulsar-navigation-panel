...
export default {
  ...
  config: {
    general: {
      ...
      properties: {
        ...
        hideTechnicals: {
          order: 4,
          title: "Hide technical headers from panel",
          type: 'boolean',
          description: 'The flag can be change faster in context menu of panel or by command',
          default: false,
        },
        hideImportants: {
          order: 5,
          title: "Hide important headers from panel",
          type: 'boolean',
          description: 'The flag can be change faster in context menu of panel or by command',
          default: false,
        },
  ...
  activate() {
    ...
    this.disposables.add(
      atom.commands.add('atom-workspace', {
        ...
        'navigation-panel:fold-technicals'  : () => this.foldAsTable('technical'),
        'navigation-panel:fold-importants'  : () => this.foldAsTable('important'),
        ...
        'navigation-panel:technicals-toggle': () => {
          value = !atom.config.get('navigation-panel.general.hideTechnicals')
          if (value) {
            atom.notifications.addSuccess('The technical headers has been hidden')
          } else {
            atom.notifications.addSuccess('The technical headers has been unhidden')
          }
          atom.config.set('navigation-panel.general.hideTechnicals', value)
        },
        'navigation-panel:importants-toggle': () => {
          value = !atom.config.get('navigation-panel.general.hideImportants')
          if (value) {
            atom.notifications.addSuccess('The technical headers has been hidden')
          } else {
            atom.notifications.addSuccess('The technical headers has been unhidden')
          }
          atom.config.set('navigation-panel.general.hideImportants', value)
        },
      }),
      ...
      atom.config.observe('navigation-panel.general.hideTechnicals', (value) => {
        if (!this.scanner) { return }
        this.scanner.hideTechnicals = value ; this.update()
      }),

      atom.config.observe('navigation-panel.general.hideImportants', (value) => {
        if (!this.scanner) { return }
        this.scanner.hideImportants = value ; this.update()
      }),
    )
  },
  ...
  foldAsTable(naviClass=null) {
    ...
    for (header of headers) {
      if (header0 && (!naviClass || (naviClass==header0.naviClass))) {
