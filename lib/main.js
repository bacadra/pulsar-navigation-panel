...
export default {
  ...
  config: {
    general: {
      ...
      properties: {
        ...
        viewMode: {
          ...
          type: 'string',
          enum: [
            {description: 'Vertical'  , value: 'block'},
            {description: 'Horizontal', value: 'flex' },
          ],
          default: 'block',
        },
        markLines: {
        ...
        collapseMode: {
          ...
          description: 'The behavior of children is to be expanded by default',
          type: 'boolean',
          default: true,
        },
        autoCollapse: {
    ...
    categories: {
      ...
      properties: {
        info: {
          order: 1,
          title: "Show info category headers by default",
          type: 'boolean',
          default: true,
        },
        success: {
          order: 2,
          title: "Show success category headers by default",
          type: 'boolean',
          default: true,
        },
        warning: {
          order: 3,
          title: "Show warning category headers by default",
          type: 'boolean',
          default: true,
        },
        error: {
          order: 4,
          title: "Show error category headers by default",
          type: 'boolean',
          default: true,
        },
        standard: {
          order: 5,
          title: "Show category-less headers by default",
          type: 'boolean',
          default: true,
        },
      },
  ...
  _refreshMarkers(headers) {
    for (var item of headers) {
      this.getMarkerLayer(item.revel).markBufferRange([item.startPoint, item.endPoint], {exclusive:true, invalidate:'inside'})
      this._refreshMarkers(item.children)
    }
  },

  getMarkerLayer(revel) {
