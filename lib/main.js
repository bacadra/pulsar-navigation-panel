    ...
    general: {
      ...
      properties: {
        ...
        viewMode: {
          order: 2,
          title: "View mode",
          description: 'Vertical style is recommended if you are using the panel for the left or right dokc, while horizontal mode is recommended for the bottom dock. Horizontal mode places all level 1 headings next to each other.',
          type: 'string',
          enum: [{value: 'block', description: 'Vertical'}, {value: 'flex', description: 'Horizontal'}],
          default: 'block',
        },
        markLines: {
          order: 3,
  ...
  activate() {
    ...
    this.disposables.add(
      atom.commands.add('atom-workspace', {
        ...
        'navigation-panel:open-and-split-left' : () => this.open({split:'left' }),
        'navigation-panel:open-and-split-right': () => this.open({split:'right'}),
        'navigation-panel:hide'                : () => this.hide(),
        'navigation-panel:toggle'              : () => this.toggle(),
        'navigation-panel:toggle-section'      : () => this.toggleSection(),
        ...
        'navigation-panel:markers-toggle'      : () => {
          if (this.markLines) {
            atom.notifications.addSuccess('The navigation markers has been deactivated')
          } else {
            atom.notifications.addSuccess('The navigation markers has been activated')
          }
          atom.config.set('navigation-panel.general.markLines', !this.markLines)
        },
        'navigation-panel:activate-all-categories': () => {
          atom.config.set(`navigation-panel.categories.info`, true)
          atom.config.set(`navigation-panel.categories.success`, true)
          atom.config.set(`navigation-panel.categories.warning`, true)
          atom.config.set(`navigation-panel.categories.error`, true)
          atom.config.set(`navigation-panel.categories.standard`, true)
        },
        'navigation-panel:deactivate-all-categories': () => {
          atom.config.set(`navigation-panel.categories.info`, false)
          atom.config.set(`navigation-panel.categories.success`, false)
          atom.config.set(`navigation-panel.categories.warning`, false)
          atom.config.set(`navigation-panel.categories.error`, false)
          atom.config.set(`navigation-panel.categories.standard`, false)
        },
        'navigation-panel:info-toggle'    : () => this.navigationView.categoryStateToggle('info'    ),
        'navigation-panel:success-toggle' : () => this.navigationView.categoryStateToggle('success' ),
        'navigation-panel:warning-toggle' : () => this.navigationView.categoryStateToggle('warning' ),
        'navigation-panel:error-toggle'   : () => this.navigationView.categoryStateToggle('error'   ),
        'navigation-panel:standard-toggle': () => this.navigationView.categoryStateToggle('standard'),
        'navigation-panel:autocollapse-toggle': () => {
          value = atom.config.get('navigation-panel.general.autoCollapse')
          if (value) {
            atom.notifications.addSuccess('Autocollapsing has been deactivated')
          } else {
            atom.notifications.addSuccess('Autocollapsing has been activated')
          }
          atom.config.set('navigation-panel.general.autoCollapse', !value)
        },
