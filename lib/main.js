...
import {NaviView}              from './navi-view'
...
export default {
    ...
    general: {
      ...
      properties: {
        ...
        smoothScroll: {
          ...
          description: 'Smoth scroll of navigation panel acc. cursor position',
          type: 'boolean',
          default: true,
        },
        updateMode: {
  ...
  activate() {
    ...
    this.disposables.add(
      atom.workspace.addOpener(uri => {
        if (uri === 'atom://navigation-panel/outline') {
          return new NaviView();
        }
      }),

      atom.contextMenu.add({'div.navigation-panel': [
        {
          label: 'Toggle pane',
          command: 'navigation-panel:toggle'
        },
        {
          label: 'Toggle markers',
          command: 'navigation-panel:toggle-markers'
        },
      ]}),

      atom.commands.add('atom-workspace', {
        'navigation-panel:toggle'        : () => this.naviView.toggle(),
        'navigation-panel:toggle-markers': () => this.markersToggle(),
        'navigation-panel:toggle-defines': () => this.sofiDefineToggle(),
      }),

      atom.commands.add('atom-text-editor', {
        'navigation-panel:fold-section-at-1': () => this.foldSection(1),
        'navigation-panel:fold-section-at-2': () => this.foldSection(2),
        'navigation-panel:fold-section-at-3': () => this.foldSection(3),
        'navigation-panel:fold-section-at-4': () => this.foldSection(4),
        'navigation-panel:fold-section-at-5': () => this.foldSection(5),
        'navigation-panel:fold-section-at-6': () => this.foldSection(6),
        'navigation-panel:fold-section-at-7': () => this.foldSection(7),
        'navigation-panel:fold-section-at-8': () => this.foldSection(8),
        'navigation-panel:fold-section-at-9': () => this.foldSection(9),

        'navigation-panel:fold-section'  : () => this.foldSection(true),
        'navigation-panel:fold-as-TOC'   : () => this.foldTOC(),
        'navigation-panel:fold-toggle'   : () => this.toggleSection(),
        'navigation-panel:unfold-current': () => this.unfold(),
      ...
      atom.config.observe('navigation-panel.general.maxHeadingDepth', () => {
        this.update(atom.workspace.getActiveTextEditor());
      }),

      atom.config.observe('navigation-panel.general.collapseByDefault', () => {
        this.update(atom.workspace.getActiveTextEditor());
      }),

      atom.config.observe('navigation-panel.general.autoScroll', () => {
        this.naviView.autoScroll = atom.config.get('navigation-panel.general.autoScroll');
      }),

      atom.config.observe('navigation-panel.general.smoothScroll', () => {
        this.naviView.smoothScroll = atom.config.get('navigation-panel.general.smoothScroll');
      }),
    )
  ...
  updateCurrentEditor(editor) {
    if (!editor) {
      ...
      // Note that we don't clear if the current panel is not a text editor,
      ...
      if (scopeIncludesOne(scopeDescriptor.scopes, SUPPORTED_SCOPES)) {
        ...
        if (this.editor!=editor) {
          ...
          if (atom.config.get('navigation-panel.general.updateMode')) {
  ...
  markersToggle() {
    if (atom.config.get('navigation-panel.general.markLines')) {
      atom.config.set('navigation-panel.general.markLines', false)
    } else {
      atom.config.set('navigation-panel.general.markLines', true)
    }
    this.update(atom.workspace.getActiveTextEditor())
  },

  sofiDefineToggle() {
    if (atom.config.get('navigation-panel.sofistik.inBlock')===true) {
      atom.config.set('navigation-panel.sofistik.inBlock', false)
    } else {
      atom.config.set('navigation-panel.sofistik.inBlock', true)
    }
    this.update(atom.workspace.getActiveTextEditor())
  },
