...
export default {
  ...
  setVisibleItem(headers, minPx, maxPx) {
    ...
    for (let header of headers) {
      let startPx = this.editorView.pixelPositionForBufferPosition(header.startPoint).top
      let endPx = this.editorView.pixelPositionForBufferPosition([header.lastRow, 0]).top
      if ((minPx<=startPx && startPx<=maxPx) || (startPx<=minPx && minPx<=endPx)) {
        header.visibility = 1
      } else {
        header.visibility = 0
      }
      this.setVisibleItem(header.children, minPx, maxPx)
    }
  },
