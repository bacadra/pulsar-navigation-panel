...
export default {
  ...
  config: {
    ...
    hide: {
      type: 'object',
      title: 'Hide',
      description: 'Hide some types of headers from panel. The flags can be change faster in context menu of panel or by command',
      properties: {
        info: {
          order: 1,
          title: "Hide info headers from panel",
          type: 'boolean',
          default: false,
        },
        success: {
          order: 2,
          title: "Hide success headers from panel",
          type: 'boolean',
          default: false,
        },
        warning: {
          order: 3,
          title: "Hide warning headers from panel",
          type: 'boolean',
          default: false,
        },
        error: {
          order: 4,
          title: "Hide error headers from panel",
          type: 'boolean',
          default: false,
        },
      },
  ...
  activate() {
    ...
    this.disposables.add(
      atom.commands.add('atom-workspace', {
        ...
        'navigation-panel:fold-greens'      : () => this.foldAsTable('green'),
        'navigation-panel:fold-reds'        : () => this.foldAsTable('red'),
        ...
        'navigation-panel:info-toggle'   : () => this.naviClassToggle('info'),
        'navigation-panel:success-toggle': () => this.naviClassToggle('success'),
        'navigation-panel:warning-toggle': () => this.naviClassToggle('warning'),
        'navigation-panel:error-toggle'  : () => this.naviClassToggle('error'),
      }),
  ...
  naviClassToggle(className) {
    value = !atom.config.get(`navigation-panel.hide.${className}`)
    if (value) {
      atom.notifications.addSuccess(`The ${className} headers has been hidden`, {icon:'fold'})
    } else {
      atom.notifications.addSuccess(`The ${className} headers has been visible`, {icon:'unfold'})
    }
    atom.config.set(`navigation-panel.hide.${className}`, value)
    this.update()
  }
