...
class AbstractModel {
  ...
  constructor(editorOrBuffer, headingRegexes) {
    ...
    this.oldHeadingStr = '';
    this.hMarks = []; // empty list for markers
  }
  ...
  // abstact parseResults(scanResult)
  ...
  **/
  _stackHeadings(rawHeadings) {
    ...
    for (let marker of this.hMarks) {marker.destroy()}
    this.hMarks = []
    ...
    for (let heading of rawHeadings) {
      ...
      if (editor && atom.config.get('navigation-pane.markLines')) {
        let marker = editor.markBufferRange(heading.headingRange, {"invalidate": "touch"})
        marker.onDidDestroy((event)=>{}).dispose()
        editor.decorateMarker(marker,
          {type: 'line', class: 'navigation-pane-mark-item-'+heading.level+' navigation-pane-mark-item'})
        this.hMarks.push(marker)

        // editor.decorateMarker(marker,
        //   {type: 'line', class: 'navigation-pane-mark-item'})
        // marker.onDidChange((event)=>{}).dispose()
      }
  ...
  _parseLevelFast(start, end, level) {
    ...
    // for each grammar.
    let limitedRegex = regex // new RegExp(regex, 'm');
