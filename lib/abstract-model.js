...
class AbstractModel {
  ...
  **/
  ...
  _parseLevelFast(start, end, level) {
    for (let deco of this.hMarks) {deco.destroy()}
    this.hMarks = []
    ...
    for (let headingRange of ranges) {
      ...
      if (result) {
        ...
        if (parsedResult) {
          ...
          // add marker ****
          if (atom.config.get('navigation-pane.markLines') && editor) {
            let marker = editor.markBufferRange(headingRange)
            this.hMarks.push(editor.decorateMarker(marker, {type: 'line',
              // class: 'navigation-pane-mark-item-'+parsedResult.level}))
              class: 'navigation-pane-mark-item'}))
          }
          // ***************
        }
      }
