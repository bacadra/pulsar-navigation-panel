...
class AbstractModel {
  ...
  parse() {
    this.flagQ = atom.config.get('navigation-pane.markLines')
    return this._parseLevel(Point.ZERO, Point.INFINITY, 1);
  }

  _stackHeadings(rawHeadings) {
    for (let lvl in this.mLayers) {this.mLayers[lvl].clear()}
    ...
    for (let heading of rawHeadings) {
      ...
      if (this.flagQ) {
        this.addOrGetMarkerLayer(heading.level).markBufferRange(heading.headingRange)
      }
    }
  ...
  addOrGetMarkerLayer(lvl) {
    if (!(lvl in this.mLayers)) {
      let lay = this.editor.addMarkerLayer()
      this.editor.decorateMarkerLayer(lay, {type: 'line', class: `navigation-pane-mark-item-${lvl} navigation-pane-mark-item`})
      this.mLayers[lvl] = lay
    }
    return this.mLayers[lvl]
  }
