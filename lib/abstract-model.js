...
class AbstractModel {
  ...
  constructor(editorOrBuffer, headingRegexes) {
    ...
    this.maxDepth = atom.config.get("navigation-pane.maxHeadingDepth");
  ...
  parse() {
    this.markLines = atom.config.get('navigation-pane.markLines')
    return this._parseLevel(Point.ZERO, Point.INFINITY, 1);
  }

  _stackHeadings(rawHeadings) {
    for (var lvl in this.markerLayers) {
      this.markerLayers[lvl].clear()
    }
    ...
    for (let heading of rawHeadings) {
      ...
      if (this.markLines) {
        this.addOrGetMarkerLayer(heading.level).markRange(
            heading.headingRange, {exclusive:true, invalidate:'inside'}
        )
      }
      // if (this.markLines) {
      //   // let marker = this.buffer.markRange(heading.headingRange)
      //   // this.editor.decorateMarker(marker, {type: 'line',
      //   //   class: `navigation-marker-${heading.level} navigation-marker`})
      //   // this.markers.push(marker)
      //
      //   // let marker = this.editor.markBufferRange(heading.headingRange)
      //   // this.editor.decorateMarker(marker, {type: 'line',
      //   //   class: `navigation-marker-${heading.level} navigation-marker`})
      //   // this.markers.push(marker)
      // }
  ...
  addOrGetMarkerLayer(lvl) {
    if (lvl in this.markerLayers) {
      return this.markerLayers[lvl]
    }
    let lay = this.markerLayers[lvl] = this.buffer.addMarkerLayer()
    this.editor.decorateMarkerLayer(lay, {
        type: 'line',
