...
export class NavigationView {
  ...
  constructor() {
    ...
    this.disposables.add(
      ...
      atom.config.observe('navigation-panel.general.autoCollapse', {}, (value) => {
        STATES.autoCollapse = value ; etch.update(this) ; if (!value) {this.collapse(3)}
      }),
  ...
  render() {
    if (STATES.searchBar) {
      searchBar = <div class='navigation-search'>{etch.dom(TextEditor, {ref:'navigationEditor', mini:true, placeholderText:'Search...'})}<div class="icon-remove-close" on={{click:this.clearQuery}}/></div>
    } else {
      searchBar = ''
    }
  ...
  clearQuery() {
    ...
    atom.views.getView(this.refs.navigationEditor).focus();
  }
...
class TreeView {
  ...
  render() {
    ...
    if (typeof STATES.collapse=='boolean') {
      this.nestedQ = !STATES.collapse
    }
