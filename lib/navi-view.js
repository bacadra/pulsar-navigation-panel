...
const STATES = { searchBar:null, categoryBar:null, autoCollapse:null, info:null, success:null, warning:null, error:null, standard:null, collapse: null }
...
export class NavigationView {
  ...
  constructor() {
    ...
    this.disposables.add(
      ...
      atom.config.observe('navigation-panel.general.autoCollapse', {}, (value) => {
        STATES.autoCollapse = value ; if (value) {etch.update(this)} else {this.collapse(3)}
      }),
      atom.config.observe('navigation-panel.general.viewMode', {}, (value) => {
        STATES.viewMode = value ; etch.update(this) }),
      ...
      atom.commands.add('atom-workspace', {
        'navigation-panel:collapse-to-current': () => this.collapse(1),
        'navigation-panel:collapse-all'       : () => this.collapse(2),
        'navigation-panel:uncollapse-all'     : () => this.collapse(3),
      }),
  ...
  render() {
    ...
    if (this.headers.length) {
      naviList = <div class="navigation-list" id='navigation-list'>{this.headers.map( (item) => { return <TreeView {...item}/> })}</div>
    } else {
  ...
  readAfterUpdate() {
    if (element = document.getElementById('navigation-list')) {
      element.style.display = STATES.viewMode
    }
    STATES.collapse = null
...
class TreeView {
  ...
  update(item) {
    this.item = item
    if (STATES.autoCollapse || STATES.collapse===1) {
      if (this.item.stackCount) {
        this.showChildren = true
      } else {
        this.showChildren = false
      }
    } else if (STATES.collapse===2) {
      this.showChildren = false
    } else if (STATES.collapse===3) {
      this.showChildren = true
    }
