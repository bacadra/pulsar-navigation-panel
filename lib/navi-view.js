...
export class NaviView {
  ...
  constructor() {
    ...
    this.outline = [];
    this.autoScroll = atom.config.get('navigation-panel:autoScroll')
    this.smoothScroll = atom.config.get('navigation-panel:smoothScroll')
    this.collapseByDefault = atom.config.get('navigation-panel.general.collapseByDefault')
  ...
  getURI() {
    return 'atom://navigation-panel/outline';
  }
  ...
  render() {
    this.outlineElements = this.outline.map(tree => {
      ...
      tree.autoCollapse = this.collapseByDefault;
      return <TreeView {...tree}/>;
    });

    return <div class="navigation-panel" id="navigation-panel">
  ...
  readAfterUpdate() {
    if (this.cursorPos) {
      ...
      for (let item of this.getDepthFirstItems(this.outline)) {
        if (item.range && item.range.containsPoint(this.cursorPos)) {
          let id = `navigation-panel-${item.range.start.row}-${item.range.end.row}`;
          ...
          if (foundElement) {
            if (!deepestItem) {
              ...
              deepestItem = foundElement;
            }
          }
        }
      }

      if (this.autoScroll && deepestItem) {
          deepestItem.scrollIntoView({behavior:this.smoothScroll ? 'smooth' : 'auto', block:'nearest', inline:'start'});
      }
    }
  }
