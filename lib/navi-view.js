...
export class NavigationView {
    ...
    this.disposables.add(
      ...
      atom.commands.add('atom-workspace', {
        ...
        'navigation-panel:auto-collapse': () => {
          STATES.visibility = STATES.collapseWork = 2 ; etch.update(this)
        },
  ...
  _filter(query, items, headers) {
    for (var item of headers) {
      item.score = zadeh.score(Diacritics.clean(item.text), query, {usePathScoring:false})
        if (item.score>0) {
          matches = query.length>0 ? zadeh.match(Diacritics.clean(item.text), query, {usePathScoring:false}) : []
          display = this.highlightMatchesInElement(item.text, matches, item.startPoint ? item.startPoint.row : null)
  ...
  highlightMatchesInElement(text, matches, row) {
    el = row!==null ? [<span class='badge badge-flexible'>{row+1}</span>] : []
    let matchedChars = []
    let lastIndex = 0
...
class TreeView {
  ...
  scrollToLine(e) {
    if (this.item.viewer) {
      this.item.viewer.scrollToDestination(this.item.dest)
