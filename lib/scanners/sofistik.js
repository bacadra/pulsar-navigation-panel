...
export class ScannerSofistik extends ScannerAbstract {
  ...
  parse(object) {
    ...
    if (this.defineCount>0) {
      ...
      if (match[6]==='+') {
        iconClass = 'prog-status status-added icon icon-diff-added'
      } else if (match[6]==='-') {
        iconClass = 'prog-status status-removed icon icon-diff-removed'
      } else {
        iconClass = 'prog-status status-modified icon icon-diff-modified'
      }
      display = [<span class={iconClass}/>, <span>{text}</span>]
    } else if (match[9]) {
      level = 6;
      text = match[9].trim()
    } else if (match[10]) {
