...
export class ScannerSofistik extends ScannerAbstract {
  ...
  parse(object) {
    ...
    if (this.defineCount>0) {
      ...
      if (match[6]==='+') {
        iconClass = 'status-added icon icon-diff-added'
      } else if (match[6]==='-') {
        iconClass = 'status-removed icon icon-diff-removed'
      } else {
        iconClass = 'status-modified icon icon-diff-modified'
      }
  ...
  toggleProg(e, object) {
    if (object.match[6]==='+') {
      ...
      e.srcElement.className = 'status-removed icon icon-diff-removed'
    } else if (object.match[6]==='-') {
      object.replace(object.matchText.replace(/\-(prog)/i, '+$1'))
      e.srcElement.className = 'status-added icon icon-diff-added'
    }
  }
