...
export class ScannerAbstract {
  ...
  constructor(editor, regex) {
    ...
    this.hideTechnicals = atom.config.get('navigation-panel.general.hideTechnicals')
    this.hideImportants = atom.config.get('navigation-panel.general.hideImportants')
  }
  ...
  getHeaders() {
    ...
    this.editor.scan(this.regex, {}, (object) => {
      ...
      if (this.hideTechnicals && item.naviClass==='technical') {
        item.hideQ = true
      } else if (this.hideImportants && item.naviClass==='important') {
        item.hideQ = true
      } else {
        item.hideQ = false
      }
